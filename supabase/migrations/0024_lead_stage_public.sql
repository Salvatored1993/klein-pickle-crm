-- Sales stage is no longer private: every salesperson can see where each
-- lead stands (alongside company name and owner). All other lead details
-- stay masked to the owning rep + admin, as set up in 0023. Same columns,
-- same order, so create or replace keeps the dependent dashboard views.
